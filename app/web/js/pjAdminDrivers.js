...
(function ($, undefined) {
	$(function () {
		...
		if ($("#grid").length > 0 && datagrid) 
		{
			...
			{
				actionOpts.push({text: myLabel.delete_selected, url: "index.php?controller=pjAdminDrivers&action=pjActionDeleteDriverBulk", render: true, confirmation: myLabel.delete_confirmation});
			}
			buttonOpts.push({
				type: "eye",
				url: "index.php?controller=pjAdminDrivers&action=pjActionGetDriverResAdminIndex&id={:id}",
				icon: "fa fa-eye"
			});

			if(pjGrid.hasUpdate)
			{
				buttonOpts.push({type: "edit", url: "index.php?controller=pjAdminDrivers&action=pjActionUpdate&id={:id}"});
			}
			if(pjGrid.hasDeleteSingle)
			{
				buttonOpts.push({type: "delete", url: "index.php?controller=pjAdminDrivers&action=pjActionDeleteDriver&id={:id}"});
			}
			...
			var $grid = $("#grid").datagrid({
				...
				dataUrl: "index.php?controller=pjAdminDrivers&action=pjActionGetDriver",
				dataType: "json",
		...
		$(document).on("click", ".btn-all", function (e) {
			...
			$grid.datagrid("load", "index.php?controller=pjAdminDrivers&action=pjActionGetDriver", "name", "ASC", content.page, content.rowCount);
