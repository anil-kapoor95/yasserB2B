...
(function ($, undefined) {
	$(function () {
		var $frmCreateFleet = $("#frmCreateFleet"),
			...
			remove_city_price_arr = new Array();
			remove_daterange_price_arr = new Array();
		...
		function setPrices()
		{
			var index_arr = new Array();
				...
				index_city_price_arr = new Array();
				index_daterange_price_arr = new Array();
			...
			$('#pjTbCityPriceTable').find(".pjTbCityPriceRow").each(function (index, row) {
				index_city_price_arr.push($(row).attr('data-index'));
			});
			$('#index_city_price_arr').val(index_city_price_arr.join("|"));

			$('#pjTbDateRangePriceTable').find(".pjTbDateRangePriceRow").each(function (index, row) {
				index_daterange_price_arr.push($(row).attr('data-index'));
			});
			$('#index_daterange_price_arr').val(index_daterange_price_arr.join("|"));

		}

		function initDateRangePicker($element) {
			if ($('#dateTimePickerOptionss').length && $element.length)
			{
				var $optionsEle = $('#dateTimePickerOptionss');
				...
				// Initialize only the passed element(s)
				$element.datetimepicker(dateOnlyOptions);
			}
		}

		// Initialize date range pickers on page load
		if ($('#dateTimePickerOptionss').length) {
			var $dateRangeInputs = $('.datetimepickdaterange');
			if ($dateRangeInputs.length > 0) {
				initDateRangePicker($dateRangeInputs);
			}
		}
		...
		$(document).on("submit", ".frm-filter", function (e) {
			...
			var $c = $("#pjTbPriceClone tbody").clone(),
				...
				$('#pjTbPriceTable').find("tbody").append(r);
		...
		}).on("click", ".btnAddCityPrice", function (e) {
			if (e && e.preventDefault) {
				e.preventDefault();
			}
			var $c = $("#pjTbCityPriceClone tbody").clone(),
				r = $c.html().replace(/\{INDEX\}/g, 'new_' + Math.ceil(Math.random() * 99999));
		
			  $('#pjTbCityPriceTable').find("tbody").append(r);
			//$(this).closest("form").find("table").find("tbody").append(r);
			
		})
		.on("click", ".btnAddDateRangePrice", function (e) {
			
			if (e && e.preventDefault) {
				e.preventDefault();
			}
			var $c = $("#pjTbDateRangePriceClone tbody").clone(),
				r = $c.html().replace(/\{INDEX\}/g, 'new_' + Math.ceil(Math.random() * 99999)),
				$newRow = $(r);
			
			$('#pjTbDateRangePriceTable').find("tbody").append($newRow);
			
			// Initialize datetimepicker for the new row's date inputs
			var $newDateInputs = $newRow.find('.datetimepickdaterange');
			initDateRangePicker($newDateInputs);
		})
		...
		.on("click", ".lnkRemovePrice", function (e) {
			...
			return false;
		}).on("click", ".lnkRemoveCityPrice", function (e) {
			...
			if(id.indexOf("new") == -1)
			{
				remove_city_price_arr.push(id);
			}
			$('#remove_city_price_arr').val(remove_city_price_arr.join("|"));
			$tr.css("backgroundColor", "#FFB4B4").fadeOut("slow", function () {
				$tr.remove();
			});			
			return false;
		}).on("click", ".lnkRemoveDateRangePrice", function (e) {
			if (e && e.preventDefault) {
				e.preventDefault();
			}
			var $tr = $(this).closest("tr");
			...
			if(id.indexOf("new") == -1)
			{
				remove_daterange_price_arr.push(id);
			}
			$('#remove_daterange_price_arr').val(remove_daterange_price_arr.join("|"));
			$tr.css("backgroundColor", "#FFB4B4").fadeOut("slow", function () {
				$tr.remove();
			});			
			return false;
