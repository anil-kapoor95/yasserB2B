...
(function ($, undefined) {
	$(function () {
		...
		{
			$.get("index.php?controller=pjAdminCategory&action=pjActionCreateForm").done(function (data) {
			...
			if ($frmCreateCategory.length > 0 && validate) {
				$frmCreateCategory.validate({
					...
					submitHandler: function(form){
					    ...
					    $.post("index.php?controller=pjAdminCategory&action=pjActionCreate", $(form).serialize()).done(function (data) {
					    	l.ladda('stop');
					    	if(data.status == 'OK')
					    	{
					    		getCreateForm();
								...
								$grid.datagrid("load", "index.php?controller=pjAdminCategory&action=pjActionGetCategory", "name", "ASC", content.page, content.rowCount);
			...
			if ($frmUpdateCategory.length > 0 && validate) {
				$frmUpdateCategory.validate({
					...
					submitHandler: function(form){
					    ...
					    $.post("index.php?controller=pjAdminCategory&action=pjActionUpdate", $(form).serialize()).done(function (data) {
					    	l.ladda('stop');
					    	if(data.status == 'OK')
					    	{
					    		getCreateForm();
		...
		if ($("#grid").length > 0 && datagrid) {
			...
			var $grid = $("#grid").datagrid({
				buttons: [{type: "edit", url: "index.php?controller=pjAdminCategory&action=pjActionUpdateForm&id={:id}"},
				          {type: "delete", url: "index.php?controller=pjAdminCategory&action=pjActionDeleteCategory&id={:id}"}
				          ],
				columns: [{text: myLabel.category_name, type: "text", sortable: true, editable: true}],
				dataUrl: "index.php?controller=pjAdminCategory&action=pjActionGetCategory",
				dataType: "json",
				fields: ['category'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "index.php?controller=pjAdminCategory&action=pjActionDeleteCategoryBulk", render: true, confirmation: myLabel.delete_confirmation}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "index.php?controller=pjAdminCategory&action=pjActionSaveExtra&id={:id}",
				select: {
					field: "id",
					name: "record[]",
