(function (window, undefined){
	...
	TaxiBooking.prototype = {
		...
		init: function (opts) {
			...
			}).on('change.tbs', '#pjTbsreturn_date', function (e) {
			    var returnDateTime = document.querySelector('.pjTbsReturn_date_time');
			    var returnDate = document.querySelector('input[name="return_date"]');
			    var returnTime = document.querySelector('input[name="return_time"]');

			    if (this.checked) {
			        this.value = '1';
			        returnDateTime.style.display = 'block';

			        // Enable and require the fields
			        returnDate.removeAttribute('disabled');
			        returnTime.removeAttribute('disabled');
			        returnDate.setAttribute('required', 'required');
			        returnTime.setAttribute('required', 'required');
			    } else {
			        this.value = '0';
			        returnDateTime.style.display = 'none';

			        // Disable and remove required + clear value
			        returnDate.setAttribute('disabled', 'disabled');
			        returnTime.setAttribute('disabled', 'disabled');
		...
		loadFleets: function () {
			...
			pjQ.$.get([this.opts.folder, "index.php?controller=pjFrontPublic&action=pjActionFleets"].join(""), params).done(function (data) {
				...
				// 	const searchResultRedirect = 'book-now';
				// 	const targetURL = `${homeURL}/${searchResultRedirect}#!/loadFleets`;

				// 	// Use sessionStorage flag to prevent infinite redirect
				// 	if (!window.location.href.includes("/book-now") && !sessionStorage.getItem("redirectedToBookNow")) {
				// 	    // Save search options before redirect
				// 	    sessionStorage.setItem("redirectedToBookNow", "1");
				// 	    window.location.href = targetURL;
				// 	} else {
				// 	    console.log('Already on target page OR already redirected, load data...');

				// 	    // Restore search options after redirect
				// 			window.location.hash = "#!/loadFleets";
				// 	    self.$container.html(data);
				...
				// 	    // Clear flag
				// 	    sessionStorage.removeItem("redirectedToBookNow");
				// 	}
	    ...
	    var end   = pjQ.$('#return_address_' + self.opts.index).val();
 		console.log(start, 'front end pickup');
		    console.log(end, 'front end retun');
	    // If empty, try to load from localStorage
	    if (!start) {
	    	
	        start = localStorage.getItem("pickup_address");
	    }
	    if (!end) {
	        end = localStorage.getItem("return_address");
	    }

	    // Save current values back to localStorage (optional)
	    if (start) {
	        localStorage.setItem("pickup_address", start);
	    }
	    if (end) {
	        localStorage.setItem("return_address", end);
	    }
	    ...
	    if (start && end) {
	        ...
	        self.directionsService.route(request, function(response, status) {
	            if (status == google.maps.DirectionsStatus.OK) {
	                ...
	                var durationInMin = Math.round(leg.duration.value / 60);

	                pjQ.$('#pjTbsDistanceFiled').val(distanceinkm);
	                pjQ.$('#pjTbsDurationInMinFiled').val(durationInMin);
	            }
	        });
	    } else {
	        self.directionsDisplay.setMap(null);
	        pjQ.$('#pjTbsDistanceFiled').val("");
	        pjQ.$('#pjTbsDurationInMinFiled').val("");
	    }
	},
		...
		//         		var durationInMin = parseInt(leg.duration.value / 60, 10);
		...
		bindSearch: function(){
...

			{
				...
				google.maps.event.addListener(autocomplete_pickup, 'place_changed', function() {
					...
					if ($from_city) {
					    pjQ.$('#from_city_' + self.opts.index).val($from_city);
					}
					self.calcRoute.call(self);
				});
			}
...

			{
				...
				google.maps.event.addListener(autocomplete_return, 'place_changed', function() {
					...
					// Set the city input once
					if ($to_city) {
					    pjQ.$('#to_city_' + self.opts.index).val($to_city);
					}
					self.calcRoute.call(self);
				});
