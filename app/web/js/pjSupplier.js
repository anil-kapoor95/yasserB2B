...
(function ($, undefined) {
	$(function () {
		...
		if ($("#grid").length > 0 && datagrid) 
		{
			...
			{
				actionOpts.push({text: myLabel.delete_selected, url: "index.php?controller=pjSupplier&action=pjActionDeleteSupplierBulk", render: true, confirmation: myLabel.delete_confirmation});
			}
			if(pjGrid.hasUpdate)
			{
				buttonOpts.push({type: "edit", url: "index.php?controller=pjSupplier&action=pjActionUpdate&id={:id}"});
			}
			if(pjGrid.hasDeleteSingle)
			{
				buttonOpts.push({type: "delete", url: "index.php?controller=pjSupplier&action=pjActionDeleteSupplier&id={:id}"});
			}
			if(pjGrid.hasRevertStatus)
			{
				actionOpts.push({text: myLabel.revert_status, url: "index.php?controller=pjSupplier&action=pjActionStatusSupplier", render: true});
			}
			if(pjGrid.hasExport)
			{
				actionOpts.push({text: myLabel.exported, url: "index.php?controller=pjSupplier&action=pjActionExportSupplier", ajax: false});
			}
			var $grid = $("#grid").datagrid({
				...
				columns: [{text: myLabel.name, type: "text", sortable: true, editable: false},
				         ...
				         {text: myLabel.phone, type: "text", sortable: false, editable: pjGrid.hasUpdate , editableWidth: 190},
				...
				dataUrl: "index.php?controller=pjSupplier&action=pjActionGetSupplier",
				dataType: "json",
				...
				saveUrl: "index.php?controller=pjSupplier&action=pjActionSaveSupplier&id={:id}",
				select: {
					field: "id",
					name: "record[]",
