	...
	$(function () {
		...
		if ($("#grid").length > 0 && datagrid) {
			...
			var $grid = $("#grid").datagrid({
				...
				paginator: {
					actions: [
					   {text: myLabel.exported, url: "index.php?controller=pjAdminSuppliers&action=pjActionExport", render: false, ajax: false},
					],
				...
				saveUrl: "index.php?controller=pjAdminSuppliers&action=pjActionSaveBooking&id={:id}",
		...
		if ($("#griddeleted").length > 0) {
			...
			var $grid = $("#griddeleted").datagrid({
				buttons: [ //{type: "print", target: "_blank", url: "index.php?controller=pjAdminSuppliers&action=pjActionPrint&id={:id}"},
					{ type: "refresh", url: "index.php?controller=pjAdminSuppliers&action=pjActionRestore&id={:id}" },
					{ type: "delete", url: "index.php?controller=pjAdminSuppliers&action=pjActionDeletePBooking&id={:id}" }
				],
				columns: [
					{ text: myLabel.client, type: "text", sortable: false },
					{ text: myLabel.fleet, type: "text", sortable: false },
		...
		$(document).on("focusin", ".timepick", function (e) {
			...
			$grid.datagrid("load", "index.php?controller=pjAdminSuppliers&action=pjActionGetBooking" + pjGrid.queryString, "created", "DESC", content.page, content.rowCount);
		...
		// 		window.open('index.php?controller=pjAdminSuppliers&action=pjActionPrint&record=' + booking_id, '_blank');
		...
		function getExtras() {
			...
			$.post("index.php?controller=pjAdminSuppliers&action=pjActionGetExtras", $frm.serialize()).done(function (data) {
		...
		function calcPrice() {
			...
			if (passengers > 0 && fleet_id > 0) {
				...
				$.post(["index.php?controller=pjAdminSuppliers&action=pjActionCalPrice"].join(""), params).done(function (data) {
